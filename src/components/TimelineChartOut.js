...
function createDataTimelineChartOut(array) {
    var data_prepared=[]
...
export default class TimelineChartOut extends Component {
  constructor(props) {
    super(props);
    this.state = {
      isLoading2: false,
  ...
  componentDidMount  = () => {
fetch('/rcv-api/public/card/5d875886-ec30-465c-9f27-67a7c5389bc0/query?parameters=%5B%7B"type"%3A"category"%2C"target"%3A%5B"variable"%2C%5B"template-tag"%2C"rcv_hub_id"%5D%5D%2C"value"%3A%22' + this.state.hub_id + '%22%7D%5D')
      .then(response => {
        if (response.ok) {
    ...
    .then(function(json) {
    const data_new = createDataTimelineChartOut(json.data.rows);
...
handleSelectCallback(chart) {
  ...
  console.log('Selected ', chart.props.rows[i][1]);
